              ...
              <ScrollArea className="h-64 rounded-md border p-4">
                <ul className="space-y-2">
                  {collection.cards.map((card, index) => (
                    <li key={card.id} className={`flex items-center text-sm ${collection.completedCardIds.has(card.id) ? 'text-primary font-semibold' : 'text-muted-foreground'}`}>
                      {collection.completedCardIds.has(card.id) ? (
                        <CheckCircle2 className="h-4 w-4 mr-3 text-green-500" />
                      ) : (
                        <span className="font-mono w-7 mr-3 text-center">{index + 1}.</span>
                      )}
